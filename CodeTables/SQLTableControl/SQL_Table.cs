...
namespace CodeTables
{
    ...
    {
        public delegate bool delegate_SetInputControls(SQLTable tbl,NavigationButtons.Navigation xnav);
        ...
        public static void DoResetSettings()
        {
            resetSettings = true;
                ...
                if (pcell[0]=='*')
                {
                    ...
                    if (sForeignKey.Length>0)
                    {
                        ...
                        if (IsForeignKey(sForeignKey, out fKey))
                        {
                            ...
                            {
                                //Column[iColumnIndex].SetType = CodeTables.Column.ValueSetTYPE.SET_MUST;
        ...
        public string MySQLcmd_CreateTable(DBTableControl dbTables, List<string> UniqueConstraintNameList, ref string sql_DBm, SQLTable DBm_owner_Table)
        {
            ...
            foreach (Column col in this.Column)
            {
                if ((col.flags & CodeTables.Column.Flags.FILTER)!=0)
                {
                    if (!col.IsIdentity)
                    {
        ...
        public string SQLcmd_CreateTable(DBTableControl dbTables, List<string> UniqueConstraintNameList, ref string sql_DBm, SQLTable DBm_owner_Table)
        {
            ...
            switch (dbTables.Con.DBType)
            {
                ...
                default:
                MessageBox.Show("Error dbTables.m_con.DBType in function:public string SQLcmd_CreateTable(CodeTables dbTables, List<string> UniqueConstraintNameList)");
                return "ERROR";
            }
        ...
        private bool InsertIntoSQLITE(DBConnection xcon,
                                      List<Col_class> lColVar,
                                      ref List<SQL_Parameter> lsqlPar,
                                      ref ID ID_returned,
                                      ref  string csError,
                                      Transaction transaction)
        {
            ...
            sqlinsert += "\n);";
            ID newID = null;
            if (transaction.ExecuteNonQuerySQLReturnID(xcon,sqlinsert, lsqlPar, ref newID, ref csError, TableName))
            {
        ...
        public bool SQLcmd_InsertInto_SQLITE(DBConnection xcon,
                                             string PrevVar,
                                             ref string sVarID,
                                             /*ref List<SQL_Parameter> lsqlPar,*/ List<SQLTable> lTable,
                                             ref bool bSomethingDefined,
                                             ref ID ID_returned,
                                             ref string csError,
                                             Transaction transaction)
        {
            ...
            for (iCol = 0; iCol < iCount; iCol++)
            {
                ...
                if (!col.IsIdentity)
                {
                    //if (col.SetType == CodeTables.Column.ValueSetTYPE.SET_MUST)
                    //{
                    ...
                    if (col.fKey != null)
                    {
                        ...
                        if (col.fKey.fTable.SQLcmd_InsertInto_SQLITE(xcon,
                                                                    sThisVar,
                                                                    ref sVarfKeyID,
                                                                    /*ref lsqlPar,*/ lTable,
                                                                    ref bSomethingDefinedInChildTable,
                                                                    ref IDfKey,
                                                                    ref csError,
                                                                    transaction))
                        {
            ...
            if ((flags & CodeTables.Column.Flags.FILTER)!=0)
            {
                string sWhere = "";
                i = 0;
                foreach (Col_class cvar in lColVar)
                {
                    if ((cvar.m_Flags & CodeTables.Column.Flags.FILTER)!=0)
                    {
                        if (i == 0)
                        {
                            sWhere += "\n WHERE \n";
                ...
                ID newID = null;
                if (transaction.ExecuteNonQuerySQLReturnID(xcon,sqlinsert.ToString(), lsqlPar, ref newID,  ref csError,TableName))
                {
                    if (ID.Validate(newID))
                    {
                        ID_returned = newID;
            ...
            {
                return InsertIntoSQLITE(xcon, lColVar, ref  lsqlPar, ref ID_returned, ref  csError, transaction);
            }
        }
...
//************************** MYSQL **************************
        ...
        private void SetReference(CodeTables.Column col, Object xTable)
        {
            Type this_class = xTable.GetType();
            FieldInfo[] this_class_fields = this_class.GetFields();
            int i;
        ...
        private void SetReference(CodeTables.Column col)
        {
        ...
        public bool SQLcmd_InsertInto(DBConnection xcon, List<SQLTable> lTable, ref string csError, StringBuilder m_strSQLUseDatabase, ref ID ID, Transaction transaction)
        {
            List<SQL_Parameter> lsqlPar = new List<SQL_Parameter>();
            if (xcon.DBType == DBConnection.eDBType.SQLITE)
            {
                ...
                if (SQLcmd_InsertInto_SQLITE(xcon, PrevVar, ref sVarID,/* ref  lsqlPar,*/ lTable,ref bSomethingDefined, ref ID, ref csError, transaction))
                {
                    if (ID.Validate(ID))
                    {
                        return true;
