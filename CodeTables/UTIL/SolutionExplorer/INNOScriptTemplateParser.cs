...
{
    public class INNOScriptTemplateParser
    {
        ...
        public string FilesToInstallBlock(ref string Err)
        {
            ...
            if (Parser.dtLibraries.Rows.Count + Parser.dtExternalDll.Rows.Count > 0)
            {
                ...
                {
                    string sFile = "\r\n  Source: \"" + dr[Parser.dcln_AssemblyName] + "\";  DestDir: \"{ app}\"";
                    sFilesBlock += sFile;
                }
                sFilesBlock += "\r\n\r\n;--- External DLLs -- \r\n";
                foreach (DataRow dr in Parser.dtExternalDll.Rows)
                {
                    string sFile = "\r\n  Source: \"" + dr[Parser.dcln_ExternalDll_absolute_path] + "\";  DestDir: \"{ app}\"";
                    sFilesBlock += sFile;
                }
                sFilesBlock += "\r\n\r\n";
                return sFilesBlock;
            ...
            {
                Err = "No files to put in INNO installer!";
                return null;
            }
        }
