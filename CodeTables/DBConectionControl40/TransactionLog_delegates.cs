...
namespace DBConnectionControl40
{
    ...
    {
        public delegate bool delegate_WriteTransactionLog_BeginTransaction(string transaction_name, ref ID Transaction_ID);

        internal delegate_WriteTransactionLog_BeginTransaction m_delegate_WriteTransactionLog_BeginTransaction = null;

        public delegate bool delegate_WriteTransactionLogExecute(ID Transaction_ID, string sql_command, List<SQL_Parameter> lpar, ID return_ID, bool bok, string error);

        internal delegate_WriteTransactionLogExecute m_delegate_WriteTransactionLogExecute = null;

        public delegate bool delegate_WriteTransactionLog_Commit(ID Transaction_ID);

        internal delegate_WriteTransactionLog_Commit m_delegate_WriteTransactionLog_Commit = null;

        public delegate bool delegate_WriteTransactionLog_Rollback(ID Transaction_ID);
