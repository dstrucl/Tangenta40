...
namespace Excell_Export_base
{
    public class Excell_base
    {
        ...
        public bool Export(ref String FileName, DataGridView dgv)
        {
            ...
            if (iRowCount > 0) 
            {
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        {
                            MessageBox.Show(static_lng_text.lng_s_ErrorStartExecuteExcel_s + ":" + FileName + "," + static_lng_text.lng_s_Error_s + " = " +Ex.Message);
                        }

                    }
                    catch (Exception Ex)
                    {
                        MessageBox.Show(static_lng_text.lng_s_ErrorInExportToExcel_s + ":" + Ex.Message);
                        m_Progress_Thread.End();
                    }
            ...
            {
                MessageBox.Show(static_lng_text.lng_s_ThereAreNoSelectedRowsToExport_s);
                return false;
            }
        }
