...
WHERE id = " + GetIDValue(Columns);
    ...
    private string AddParam(ValSet valSet, object obj_new)
    {
        ...
        m_UpdateAllPar.Add(new DBConnectionControl40.SQL_Parameter(sParName, valSet.col_type.m_Type, false, obj_new));
        return sParName;
    }
            ...
            if (colvs.Where.enabled) //CHANGE!!
            {
                if (colvs.Where.expression == null)
                {
                    if (!bFirstFound)
                    {
                        ...
                        lPar.Add(new DBConnectionControl40.SQL_Parameter("@par_" + GetParamName(colvs.col_name), colvs.col_type.m_Type, false, colvs.obj));
                        bFirstFound = true;
                    }
                    else
                    {
            ...
            if (colvs.Where.enabled) //CHANGE!!
            {
                if (colvs.Where.expression == null)
                {
                    if (!bFirstFound)
                    {
                        ...
                        lPar.Add(new DBConnectionControl40.SQL_Parameter("@par_" + Func.CleanName(Columns[i].col_name), Columns[i].col_type.m_Type, false, Columns[i].obj));
                        bFirstFound = true;
                    }
                    else
                    {
