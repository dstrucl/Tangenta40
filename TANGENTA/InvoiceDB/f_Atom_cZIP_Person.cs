...
namespace InvoiceDB
{
    public static class f_Atom_cZIP_Person
    {
        ...
        internal static bool Get(string_v zIP_v, ref long_v atom_cZIP_Person_ID_v)
        {
            if (zIP_v != null)
            {
                ...
                if (DBSync.DBSync.ReadDataTable(ref dt, sql, lpar, ref Err))
                {
                    ...
                    else
                    {
                        ...
                        {
                            LogFile.Error.Show("ERROR:InvoiceDB:f_Atom_cZIP_Person:Get(string_v zIP_v, ref long_v atom_cZIP_Person_ID_v) sql=" + sql + "\r\nErr=" + Err);
                            return false;
                        }
                    }
                }
            ...
            {
                LogFile.Error.Show("ERROR:InvoiceDB:f_Atom_cZIP_Person:Get(string_v zIP_v, ref long_v atom_cZIP_Person_ID_v) zIP_v may not be null!");
                return false;
            }
        }
    }
