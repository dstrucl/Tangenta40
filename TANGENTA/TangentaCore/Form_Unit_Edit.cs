...
namespace TangentaCore
{
    ...
    {
        private DataTable dt_Unit = new DataTable();
        private CodeTables.DBTableControl dbTables = null;
        private SQLTable tbl = null;
        private string ColumnOrderBy = null;
        private NavigationButtons.Navigation nav = null;

        public Form_Unit_Edit(CodeTables.DBTableControl xdbTables, SQLTable xtbl, string xColumnOrderBy, NavigationButtons.Navigation xnav)
        {
            InitializeComponent();
            nav = xnav;
            dbTables = xdbTables;
        ...
        private bool CheckUnitData(ref string Err)
        {
            ...
            if (dbTables.Con.ReadDataTable(ref dt, sql_Unit, ref Err))
            {
                int Count;
                Count = dt.Rows.Count;
                if (Count > 0)
        ...
        private void Form_Unit_Edit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!CheckUnitData(ref Err))
            {
                if (Err == null)
                {
                    if (MessageBox.Show(this, lng.s_UnitTableHasNoData_YouMustEnterData_close_anyway.s,"?",MessageBoxButtons.YesNo,MessageBoxIcon.Question,MessageBoxDefaultButton.Button2)== DialogResult.No)
                    {
                        e.Cancel = true;
                    }
                }
