    ...
    public partial class STDUserInfoForm : Form
    {
        ...
        public STDUserInfoForm(STD xstd)
        {
            ...
            try
            {
                ...
                txt_UserName.Text = std.lctrl.awp.LMOUser_Single.UserName;
                txt_first_name.Text = std.lctrl.awp.LMOUser_Single.FirstName;
                txt_last_name.Text = std.lctrl.awp.LMOUser_Single.LastName;
                txt_Identiy.Text = std.lctrl.awp.LMOUser_Single.Identity;
                txt_Contact.Text = std.lctrl.awp.LMOUser_Single.Email;
                chk_NotActiveAfterPasswordExpires.Checked = std.lctrl.awp.LMOUser_Single.NotActiveAfterPasswordExpires;
                chk_PasswordExpiresInNumberOfDays.Checked = std.lctrl.awp.LMOUser_Single.bPasswordExpiresInNumberOfDays;
                chk_PasswordNeverExpires.Checked = std.lctrl.awp.LMOUser_Single.PasswordNeverExpires;
                txt_NumberOfDays.Text = std.lctrl.awp.LMOUser_Single.Maximum_password_age_in_days.ToString();
                txt_LastUserPasswordDefinitionTime.Text = std.lctrl.awp.LMOUser_Single.LastUserPasswordDefinitionTime.ToString();
                if (!std.lctrl.awp.LMOUser_Single.PasswordNeverExpires)
                {
        ...
        {
            STDLoginHistoryForm login_history_form = new STDLoginHistoryForm(std,Convert.ToInt32(std.lctrl.awp.LMOUser_Single.LoginUsers_id), std.lctrl.awp.LMOUser_Single.UserName);
