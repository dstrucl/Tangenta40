...
public class Where : bEnabled
{
    ...
    public List<DBConnectionControl4.SQL_Parameter> m_expression_parameters = null;
    public string expression
    {
        get { return m_expression; }
        set { m_expression = value; }
    }
    public void AddParameter(DBConnectionControl4.SQL_Parameter par)
    {
        if (m_expression_parameters == null)
        {
            m_expression_parameters = new List<DBConnectionControl4.SQL_Parameter>();
        }
        m_expression_parameters.Add(par);
    }

    internal void Add_lPar(List<DBConnectionControl4.SQL_Parameter> lPar)
    {
        if (m_expression_parameters != null)
        {
            foreach (DBConnectionControl4.SQL_Parameter par in m_expression_parameters)
            {
                lPar.Add(par);
            }
        }
...
public class XTable
{
    ...
    private List<DBConnectionControl4.SQL_Parameter> m_UpdateAllPar = new List<DBConnectionControl4.SQL_Parameter>();
    public delegate void delegate_UpdateObjects(DataRow dr);
    protected delegate_UpdateObjects myUpdateObjects = null;
    protected string tablename;
    public DBConnectionControl4.DBConnection m_con;
...
WHERE id = " + GetIDValue(Columns);
    ...
    private string AddParam(ValSet valSet, object obj_new)
    {
        ...
        m_UpdateAllPar.Add(new DBConnectionControl4.SQL_Parameter(sParName, valSet.col_type.m_Type, false, obj_new));
        return sParName;
    }
    ...
    public bool read(ref string csError)
    {
        bRead = true;
        dt.Clear();
        List<DBConnectionControl4.SQL_Parameter> lPar = new List<DBConnectionControl4.SQL_Parameter>();
            ...
            if (colvs.Where.enabled) //CHANGE!!
            {
                if (colvs.Where.expression == null)
                {
                    if (!bFirstFound)
                    {
                        ...
                        lPar.Add(new DBConnectionControl4.SQL_Parameter("@par_" + GetParamName(colvs.col_name), colvs.col_type.m_Type, false, colvs.obj));
                        bFirstFound = true;
                    }
                    else
                    {
            ...
            if (colvs.Where.enabled) //CHANGE!!
            {
                if (colvs.Where.expression == null)
                {
                    if (!bFirstFound)
                    {
                        ...
                        lPar.Add(new DBConnectionControl4.SQL_Parameter("@par_" + Func.CleanName(Columns[i].col_name), Columns[i].col_type.m_Type, false, Columns[i].obj));
                        bFirstFound = true;
                    }
                    else
                    {
...
public class XTableFunction
{
    ...
    public List<DBConnectionControl4.SQL_Parameter> FuncParams = new List<DBConnectionControl4.SQL_Parameter>();
    ...
    public bool read(ref string csError)
    {
        ...
        // Parameters part
        string sql_args = "";
        foreach (DBConnectionControl4.SQL_Parameter par in FuncParams)
        {
            lPar.Add(par);
...
public class XProcedure
{
    ...
    public List<DBConnectionControl4.SQL_Parameter> ProcParams = new List<DBConnectionControl4.SQL_Parameter>();
    ...
    public object ProcParamValue(string name)
    {
        ...
        {
            DBConnectionControl4.SQL_Parameter par = ProcParams[iPar];
