...
namespace Tangenta
{
    public class ShopShelf_Source
    {
        ...
        internal void Add_Stock_Data(usrc_Item xusrc_Item, bool b_from_factory)
        {
            if (b_from_factory)
            {
                ...
                stock_data.dQuantity.v = xusrc_Item.nmUpDn_FactoryQuantity.Value;        
                Stock_Data_List.Add(stock_data);
            }
            else
            {
                decimal dquantity = xusrc_Item.nmUpDn_StockQuantity.Value;
                xusrc_Item.m_Item_Data.Stock_Data_List.Sort((x, y) => x.Stock_ExpiryDate.v.CompareTo(y.Stock_ExpiryDate.v));
                foreach (Stock_Data sd in xusrc_Item.m_Item_Data.Stock_Data_List)
                {
                    if (dquantity>0)
                    {
