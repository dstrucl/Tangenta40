...
namespace Tangenta
{
    ...
    {
        private DataTable dt_DBSettings = new DataTable();
        ...
        private void Form_DBSettings_Edit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!CheckDBSettingsData(ref Err))
            {
                if (Err == null)
                {
                    if (MessageBox.Show(this, lng.s_DBSettingsTableHasNoData_YouMustEnterData_close_anyway.s,"?",MessageBoxButtons.YesNo,MessageBoxIcon.Question,MessageBoxDefaultButton.Button2)== DialogResult.No)
                    {
                        e.Cancel = true;
                    }
                }
