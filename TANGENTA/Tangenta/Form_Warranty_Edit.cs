    ...
    public partial class Form_Warranty_Edit : Form
    {
        ...
        private void Form_Warranty_Edit_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (!CheckWarrantyData(ref Err))
            {
                if (Err == null)
                {
                    if (MessageBox.Show(this, lng.s_WarrantyTableHasNoData_YouMustEnterData_close_anyway.s,"?",MessageBoxButtons.YesNo,MessageBoxIcon.Question,MessageBoxDefaultButton.Button2)== DialogResult.No)
                    {
                        e.Cancel = true;
                    }
                }
        ...
        private void Form_Warranty_Edit_KeyUp(object sender, KeyEventArgs e)
        {
