...
namespace TangentaBooting
{
    public class Booting_03_Check_DBConnection
    {
        ...
        public Booting_03_Check_DBConnection(Form xfmain, Startup.Startup x_sturtup)
        {
            frm = xfmain;
            m_startup = x_sturtup;
        ...
        {
            bDatabaseReset = Reset2FactorySettings.DBConnectionControlXX_EXE;
            if (CommandLineParam.bChangeConnection)
            {
                bDatabaseReset = true;
            }
            return new startup_step(lng.s_Startup_Check_DBConnection.s, m_startup, Booting.nav,
                                                                                    ...
                                                                                    ref string Err)
        {
            switch (eExitResult)
            {
                case Navigation.eEvent.NEXT:
                    ...
                    Err = "ERROR:Tangenta_Form:Startup_03_onformresult_Show_TestConnectionForm:form is not of type " + typeof(DBConnectionControl40.TestConnectionForm).ToString();
                ...
                case Navigation.eEvent.NEXT:
                    if (form is DBConnectionControl40.ConnectionDialog)
                    {
                        ...
                        if (DBSync.DBSync.RemoteDB_data != null)
                        {
                            ...
                            if (DBSync.DBSync.RemoteDB_data.Save(Booting.CodeTables_IniFileFolder,ref Err))
                            {
                                return Startup_onformresult_proc_Result.DO_CHECK_PROC_AGAIN;
                            }
                        }
                    ...
                    {
                        Err="ERROR:Tangenta:Form:Startup_03_onformresult_ShowDBConnnection:form = " + form.GetType().ToString() + " is not implemented!";
                        return Startup_onformresult_proc_Result.ERROR;
                    }
