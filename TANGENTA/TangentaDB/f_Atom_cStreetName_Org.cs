...
namespace TangentaDB
{
    ...
    {
        public static bool Get(ID cStreetName_Org_ID,ref ID Atom_cStreetName_Org_ID, Transaction transaction)
        {
            string Err = null;
                ...
                if (dt.Rows.Count>0)
                {
                    ...
                    if (o_StreetName is string)
                    {
                        ...
                        if (DBSync.DBSync.ReadDataTable(ref dt, sql,lpar, ref Err))
                        {
                            if (dt.Rows.Count > 0)
                            {
                                if (Atom_cStreetName_Org_ID==null)
                                {
                                    Atom_cStreetName_Org_ID = new ID();
                                }
                                Atom_cStreetName_Org_ID.Set(dt.Rows[0]["ID"]);
                                return true;
                            }
                            else
                            {
                                sql = @"insert into Atom_cStreetName_Org (StreetName) values (" + sval_StreetName+")";
                                if (transaction.ExecuteNonQuerySQLReturnID(DBSync.DBSync.Con,sql, lpar, ref Atom_cStreetName_Org_ID,  ref Err, "Atom_cStreetName_Org"))
                                {
                                    return true;
                                }
                                else
        ...
        internal static bool Get(string_v streetName_v, ref ID atom_cStreetName_Org_ID, Transaction transaction)
        {
            if (streetName_v != null)
            {
                ...
                if (DBSync.DBSync.ReadDataTable(ref dt, sql, lpar, ref Err))
                {
                    ...
                    else
                    {
                        ...
                        if (transaction.ExecuteNonQuerySQLReturnID(DBSync.DBSync.Con,sql, lpar, ref xAtom_cStreetName_Org_ID,  ref Err, "Atom_cStreetName_Org"))
                        {
                            if (atom_cStreetName_Org_ID == null)
                            {
                                atom_cStreetName_Org_ID = new ID();
                            }
                            atom_cStreetName_Org_ID.Set(xAtom_cStreetName_Org_ID);
                            return true;
                        }
                        else
                        {
                            LogFile.Error.Show("ERROR:TangentaDB:f_Atom_cStreetName_Org:Get(string_v streetName_v, ref ID atom_cStreetName_Org_ID) sql=" + sql + "\r\nErr=" + Err);
                            return false;
                        }
                    }
                }
            ...
            {
                LogFile.Error.Show("ERROR:TangentaDB:f_Atom_cStreetName_Org:Get(string_v streetName_v, ref ID atom_cStreetName_Org_ID) streetName_v may not be null!");
                return false;
            }
        }
    }
