...
namespace TangentaDB
{
    ...
    {
        public static bool Get(string Image_Hash, byte[] Image_Data, ref long SimpleItem_Image_ID)
        {
            ...
            if (DBSync.DBSync.ReadDataTable(ref dt, sql, lpar, ref Err))
            {
                if (dt.Rows.Count > 0)
                {
                    SimpleItem_Image_ID = (long)dt.Rows[0]["ID"];
                    return true;
                }
                else
                {
                    ...
                    object oret = null;
                    if (DBSync.DBSync.ExecuteNonQuerySQLReturnID(sql, lpar, ref SimpleItem_Image_ID, ref oret, ref Err, "SimpleItem_Image"))
                    {
                        return true;
                    }
                    else
        ...
        public static bool Get(Image SimpleItem_Image, ref long SimpleItem_Image_ID)
        {
