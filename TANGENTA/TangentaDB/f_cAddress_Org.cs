        ...
        public static bool DeleteAll()
        {
            ...
            switch (DBSync.DBSync.m_DBType)
            {
                case DBConnection.eDBType.SQLITE:
                    sql_delete = @"delete from cAddress_Org;
                                  delete from sqlite_sequence where name = 'cAddress_Org";
                    break;
                case DBConnection.eDBType.MSSQL:
                    sql_delete = @"delete from cAddress_Org;
                                   DBCC CHECKIDENT ('[cAddress_Org]', RESEED, 0);";
                    break;
            }
            object oret = null;
            ...
            {
                LogFile.Error.Show("ERROR:f_cAddress_Org:DeleteAll:sql=" + sql_delete + "\r\nErr=" + Err);
                return false;
            }
        }
    }
