...
{
    [ToolboxBitmap("E:\\ManualReader\\ctlogina\\SQLTableControl\\Resources\\SQLTableControl.ico")]
            ...
            foreach (string stbl in table_names)
            {
                foreach (SQLTable tbl in items)
                {
                    if (stbl.ToLower().Equals(tbl.TableName.ToLower()))
                    {
                        ...
                        {
                            foreach (SQLTableControl.Column col in tbl.Column)
                            {
        ...
        public StringBuilder SQLcmd_DropAll_Views(DBConnection xcon)
        {
          ...
          //  int iTable;
            ...
            foreach (DataBaseView dbv in SQL_DataBase_VIEW_List)
            {
                ...
                {
                    LogFile.Error.Show("ERROR:SQLTableControl:DBTableControl:SQLcmd_DropAllTables:Not valid xcon.DBType : " + xcon.DBType.ToString());
                }
                strSqlAll.Append(strTable);
            }
            return strSqlAll;
