...
{
    public class SelectViewListBox : ListBox
    {
        ...
        protected override void OnDrawItem(DrawItemEventArgs e)
        {
            ...
            if (state==RadioButtonState.CheckedNormal)
            {
                textBrush = SystemBrushes.MenuHighlight;
            }
            ...
            //DrawImage at location
            Image img = SQLTableControl.Properties.Resources.SelectViewIcon.ToBitmap();
            SizeF sz = img.PhysicalDimension;
            e.Graphics.DrawImage(img, e.Bounds.X + 1, (e.Bounds.Bottom + e.Bounds.Top) / 2 - sz.Height / 2);
            ...
            if (state == RadioButtonState.CheckedNormal)
            {
                ...
                {
                    if (this.Items[e.Index].GetType() == typeof(ViewXml))
                    {
                        ViewXml xViewXml = (ViewXml)this.Items[e.Index];
                        e.Graphics.DrawString(xViewXml.Name, e.Font, textBrush, bounds, this.Align);
                    }
                }
            }
            else
