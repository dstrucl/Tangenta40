...
namespace Tangenta
{
    public class Atom_ProformaInvoice_Price_Item_Stock_Data
    {
        ...
        internal void Set(DataRow dria,
        ...
        internal void Set(usrc_Item xusrc_Item, bool b_from_factory)
        {
            m_ShopShelf_Source.Clear();
            Atom_ProformaInvoice_Price_Item_Stock_ID = null;
            ProformaInvoice_ID = new long_v();
            ProformaInvoice_ID.v = xusrc_Item.m_usrc_Atom_ItemsList.m_InvoiceDB.m_CurrentInvoice.ProformaInvoice_ID;
            Atom_Price_Item_ID = null; // func.set_long(dria["Atom_Price_Item_ID"]);


            RetailPricePerUnit = (decimal_v)func.Copy(xusrc_Item.m_Item_Data.RetailPricePerUnit); //func.set_decimal(dr[cpis.icol_RetailPricePerUnit]);

            Discount = (decimal_v)func.Copy(xusrc_Item.m_Item_Data.Price_Item_Discount); // func.set_decimal(dr[cpis.icol_Discount]);
            ...
            ExtraDiscount = func.set_decimal(xusrc_Item.ExtraDiscount);
            Atom_Item_UniqueName = (string_v)func.Copy(xusrc_Item.m_Item_Data.Item_UniqueName);// func.set_string(dr[cpis.icol_Item_UniqueName]);

            Atom_Item_Name_Name = (string_v)func.Copy(xusrc_Item.m_Item_Data.Item_Name);//func.set_string(dr[cpis.icol_Item_Name]);
            Atom_Item_barcode_barcode = (string_v)func.Copy(xusrc_Item.m_Item_Data.Item_barcode);//func.set_string(dr[cpis.icol_Item_barcode]);
            Atom_Taxation_Name = (string_v)func.Copy(xusrc_Item.m_Item_Data.Taxation_Name);//func.set_string(dr[cpis.icol_Taxation_Name]);
            Atom_Taxation_Rate = (decimal_v)func.Copy(xusrc_Item.m_Item_Data.Taxation_Rate);//func.set_decimal(dr[cpis.icol_Taxation_Rate]);
            Atom_Item_Description_Description = (string_v)func.Copy(xusrc_Item.m_Item_Data.Item_Description);//func.set_string(dr[cpis.icol_Item_Description]);
            Atom_Warranty_ID = null; //func.set_long(dria["Atom_Warranty_ID"]);
            Atom_Warranty_WarrantyDurationType = (short_v)func.Copy(xusrc_Item.m_Item_Data.Warranty_WarrantyDurationType);//func.set_short(dr[cpis.icol_Warranty_WarrantyDurationType]);
            Atom_Warranty_WarrantyDuration = (int_v)func.Copy(xusrc_Item.m_Item_Data.Warranty_WarrantyDuration);//func.set_int(dr[cpis.icol_Warranty_WarrantyDuration]);
            Atom_Warranty_WarrantyConditions = (string_v)func.Copy(xusrc_Item.m_Item_Data.Warranty_WarrantyConditions);//func.set_string(dr[cpis.icol_Warranty_WarrantyConditions]);
            Atom_Expiry_ID = null; //func.set_long(dr["Atom_Expiry_ID"]);
            Atom_Expiry_ExpectedShelfLifeInDays = (int_v)func.Copy(xusrc_Item.m_Item_Data.Expiry_ExpectedShelfLifeInDays);//func.set_int(dr[cpis.icol_Expiry_ExpectedShelfLifeInDays]);
            Atom_Expiry_SaleBeforeExpiryDateInDays = (int_v)func.Copy(xusrc_Item.m_Item_Data.Expiry_SaleBeforeExpiryDateInDays);//func.set_int(dr[cpis.icol_Expiry_SaleBeforeExpiryDateInDays]);
            Atom_Expiry_DiscardBeforeExpiryDateInDays = (int_v)func.Copy(xusrc_Item.m_Item_Data.Expiry_DiscardBeforeExpiryDateInDays);//func.set_int(dr[cpis.icol_Expiry_DiscardBeforeExpiryDateInDays]);
            Atom_Expiry_ExpiryDescription = (string_v)func.Copy(xusrc_Item.m_Item_Data.Expiry_Description);//func.set_string(dr[cpis.icol_Expiry_ExpiryDescription]);
            Item_ID = (long_v)func.Copy(xusrc_Item.m_Item_Data.Item_ID);//func.set_long(dr["Item_ID"]);
            Atom_Unit_Name = (string_v)func.Copy(xusrc_Item.m_Item_Data.Unit_Name);//func.set_string(dr[cpis.icol_Unit_Name]);
            Atom_Unit_Symbol = (string_v)func.Copy(xusrc_Item.m_Item_Data.Unit_Symbol);//func.set_string(dr[cpis.icol_Unit_Symbol]);
            Atom_Unit_DecimalPlaces = (int_v)func.Copy(xusrc_Item.m_Item_Data.Unit_DecimalPlaces);//func.set_int(dr[cpis.icol_Unit_DecimalPlaces]);
            Atom_Unit_Description = (string_v)func.Copy(xusrc_Item.m_Item_Data.Unit_Description);//func.set_string(dr[cpis.icol_Unit_Description]);
            Atom_Unit_StorageOption = (bool_v)func.Copy(xusrc_Item.m_Item_Data.Unit_StorageOption);//func.set_bool(dr[cpis.icol_Unit_StorageOption]);
            Atom_PriceList_Name = (string_v)func.Copy(xusrc_Item.m_Item_Data.PriceList_Name);//func.set_string(dr[cpis.icol_PriceList_Name]);
            Atom_Currency_Name = (string_v)func.Copy(xusrc_Item.m_Item_Data.Currency_Name);// func.set_string(dr[cpis.icol_Currency_Name]);
            Atom_Currency_Abbreviation = (string_v)func.Copy(xusrc_Item.m_Item_Data.Currency_Abbreviation);//func.set_string(dr[cpis.icol_Currency_Abbreviation]);
            Atom_Currency_Symbol = (string_v)func.Copy(xusrc_Item.m_Item_Data.Currency_Symbol);//func.set_string(dr[cpis.icol_Currency_Symbol]);
            Atom_Currency_DecimalPlaces = (int_v)func.Copy(xusrc_Item.m_Item_Data.Currency_DecimalPlaces);//func.set_int(dr[cpis.icol_Currency_DecimalPlaces]);
            Atom_Item_Image_Hash = (string_v)func.Copy(xusrc_Item.m_Item_Data.Item_Image_Image_Hash);//func.set_string(dr[cpis.icol_Currency_Symbol]);
            Atom_Item_Image_Data = (byte_array_v)func.Copy(xusrc_Item.m_Item_Data.Item_Image_Image_Data);//Itemfunc.set_byte_array(dr[cpis.icol_Item_Image_Image_Data]);
            s1_name = xusrc_Item.m_Item_Data.s1_name;
            s2_name = xusrc_Item.m_Item_Data.s2_name;
            s3_name = xusrc_Item.m_Item_Data.s3_name;

            m_ShopShelf_Source.Add_Stock_Data(xusrc_Item, b_from_factory);
        }
    }
}
