...
namespace Tangenta
{
    public class ShopShelf
    {
        ...
        internal bool Load(long m_PriceList_ID, string[] s_name_Group)
        {
        ...
        internal bool GetGroupsTable(long PriceList_ID)
        {
            string sql = @"select 
              s1.Name as s1_name,
              s2.Name as s2_name,
        ...
        internal int GetIndex(Atom_ProformaInvoice_Price_Item_Stock_Data appisd)
        {
            foreach (object o in items)
            {
        ...
        internal void Set_dQuantity_New_InStock(long stock_id, decimal dQuantity_New_InStock)
        {
