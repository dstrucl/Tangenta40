...
struct LittleEndian
{
	...
	static void Read_EXCEL_ALIGNMENT(const vector<char>& buffer, EXCEL_ALIGNMENT retVal, size_t pos=0, int bytes=0)
	{
		BYTE xretVal;
		Read(buffer,xretVal,pos,bytes);
		retVal = (EXCEL_ALIGNMENT)xretVal;
	}


	static void Read_EXCEL_FONT_OPTIONS(const vector<char>& buffer, EXCEL_FONT_OPTIONS& retVal, size_t pos=0, int bytes=0)
	{
		short xRetVal;
		Read(buffer, xRetVal, pos, bytes);
		retVal = (EXCEL_FONT_OPTIONS)xRetVal;
	}
	
	static void Read_EXCEL_ESCAPEMENT(const vector<char>& buffer, EXCEL_ESCAPEMENT& retVal, size_t pos=0, int bytes=0)
	{
		short xRetVal;
		Read(buffer, xRetVal, pos, bytes);
		retVal = (EXCEL_ESCAPEMENT)xRetVal;
	}
	
	static void Read_EXCEL_UNDERLINE_FLAGS(const vector<char>& buffer, EXCEL_UNDERLINE_FLAGS& retVal, size_t pos=0, int bytes=0)
	{
		char xRetVal;
		Read(buffer, xRetVal, pos, bytes);
		retVal = (EXCEL_UNDERLINE_FLAGS)xRetVal;
	}

	static void Read_EXCEL_FONT_FAMILY(const vector<char>& buffer, EXCEL_FONT_FAMILY& retVal, size_t pos=0, int bytes=0)
	{
		char xRetVal;
		Read(buffer, xRetVal, pos, bytes);
		retVal = (EXCEL_FONT_FAMILY)xRetVal;
	}
	...
	static void Write_EXCEL_ALIGNMENT(vector<char>& buffer, EXCEL_ALIGNMENT val, size_t pos=0, int bytes=0)
	{
		BYTE xval = (BYTE)val;
		Write(buffer,xval,pos,bytes);
	}

	static void Write_EXCEL_FONT_OPTIONS(vector<char>& buffer, EXCEL_FONT_OPTIONS val, size_t pos=0, int bytes=0)
	{
		short xval = (short)val;
		Write(buffer,xval,pos,bytes);
	}

	static void Write_EXCEL_ESCAPEMENT(vector<char>& buffer, EXCEL_ESCAPEMENT val, size_t pos=0, int bytes=0)
	{
		short xval = (short)val;
		Write(buffer,xval,pos,bytes);
	}
	...
	static void Write_EXCEL_UNDERLINE_FLAGS(vector<char>& buffer, EXCEL_UNDERLINE_FLAGS val, size_t pos=0, int bytes=0)
	{
		char xval = (char)val;
		Write(buffer,xval,pos,bytes);
	}
	
	static void Write_EXCEL_FONT_FAMILY(vector<char>& buffer, EXCEL_FONT_FAMILY val, size_t pos=0, int bytes=0)
	{
		char xval = (char)val;
		Write(buffer,xval,pos,bytes);
	}
...
public:
	...
	struct Font : public Record
	{
		...
		virtual ULONG RecordSize();
		USHORT height_;
		EXCEL_FONT_OPTIONS options_;
		USHORT colourIndex_;
		USHORT weight_;
		EXCEL_ESCAPEMENT escapementType_;
		EXCEL_UNDERLINE_FLAGS underlineType_;
		EXCEL_FONT_FAMILY family_;
		BYTE characterSet_;
		BYTE unused_;
		SmallString name_;
	};
	...
	struct XF : public Record
	{
		...
		EXCEL_ALIGNMENT alignment_;	// 0x08: 1 = Text is wrapped at right border
		BYTE rotation_;
...
public: // Cell functions
	...
	void SetRowHeight(const int rowindex , const USHORT rowheight);
...
public:
	...
	void SetFormat(const ExcelFormat::oCellFormat& fmt); //MF

	void SetFormat(const ExcelFormat::oCellFormat *pfmt); //MF
...
//	void SetXFormatIdx(int xf_idx)			{_xf_idx = xf_idx;} //MF
//
//	void SetFormat(const oCellFormat& fmt) {_xf_idx = fmt.get_xf_idx();} //MF
//
//	void EraseContents();	///< Erase the content of current Excel cell. Set type to UNDEFINED.
//
//	USHORT GetMergedRows() const				{return mergedRows_;}
...
ref class CellFormat;

public ref class ExcelCell
{
	class oExcelCell *m_oExcelCell;

public:
	ExcelCell() {m_oExcelCell = new oExcelCell();}
	~ExcelCell() {delete m_oExcelCell;}

	void Attach(oExcelCell  *poExcelCell)
		{
			m_oExcelCell = poExcelCell;
		}

	enum class ExcelCell_ENUM {UNDEFINED, INT, DOUBLE, STRING, WSTRING, FORMULA/*MF*/};

	int Type() {return m_oExcelCell->Type();}					///< Get type of value stored in current Excel cell. Returns one of the above enums.

	bool Get(int& val)  {return m_oExcelCell->Get(val);}			///< Get an integer value. Returns false if cell does not contain an integer or a double.
	bool Get(double& val)  {return m_oExcelCell->Get(val);}		///< Get a double value. Returns false if cell does not contain a double or an integer.

	size_t GetStringLength()  {return m_oExcelCell->GetStringLength();} 	///< Return length of ANSI or Unicode string (excluding null character).

	int GetInteger()  {return m_oExcelCell->GetInteger();} 			///< Get an integer value. Returns 0 if cell does not contain an integer.
	double GetDouble()  {return m_oExcelCell->GetDouble();}			///< Get a double value. Returns 0.0 if cell does not contain a double.
	const char* GetString()  {return m_oExcelCell->GetString();}		///< Get an ANSI string. Returns 0 if cell does not contain an ANSI string.
	const wchar_t* GetWString()  {return m_oExcelCell->GetWString();}	///< Get an Unicode string. Returns 0 if cell does not contain an Unicode string.

	// friend ostream& operator<<(ostream& os, const oExcelCell& cell);	///< Print cell to output stream. Print a null character if cell is undefined.

	void Set(int val)  {m_oExcelCell->Set(val);}					///< Set content of current Excel cell to an integer.
	void Set(double val){m_oExcelCell->Set(val);}				///< Set content of current Excel cell to a double.
	void Set(const char* str) {m_oExcelCell->Set(str);}			///< Set content of current Excel cell to an ANSI string.
	void Set(const wchar_t* str){m_oExcelCell->Set(str);}		///< Set content of current Excel cell to an Unicode string.
	void Set(System::DateTime ^sDateTime)
	{
		double dt;
		dt = sDateTime->ToOADate();
		Set(dt);
	}

	void SetTimeSpan(System::TimeSpan ^sTimeSpan)
	{
		double dt;
		dt = sTimeSpan->TotalDays;
		Set(dt);
	}
	...
	void Set(System::String ^str){
		  // Convert to a wchar_t*
								...
								m_oExcelCell->Set(wcstring);
																 }		///< Set content of current Excel cell to an Unicode string.

	void SetInteger(int val) {m_oExcelCell->Set(val);}			///< Set content of current Excel cell to an integer.
	void SetDouble(double val) {m_oExcelCell->SetDouble(val);} 		///< Set content of current Excel cell to a double.
	void SetRKValue(int rkValue){m_oExcelCell->SetRKValue(rkValue);}		///< Set content of current Excel cell to a double or integer value.
	void SetString(const char* str){m_oExcelCell->SetString(str);}	///< Set content of current Excel cell to an ANSI string.
	void SetWString(const wchar_t* str){m_oExcelCell->SetWString(str);}///< Set content of current Excel cell to an Unicode string.

	void SetFormula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f) {m_oExcelCell->SetFormula(f);} //MF

	int GetXFormatIdx() 				{return m_oExcelCell->GetXFormatIdx();}	//MF
	void SetXFormatIdx(int xf_idx)			{m_oExcelCell->SetXFormatIdx(xf_idx);} //MF
	//void SetFormat(const ExcelFormat::oCellFormat& fmt) {m_oExcelCell->SetFormat( fmt);} //MF
	
	void SetFormat(CellFormat ^fmt); //MF

	void EraseContents() {m_oExcelCell->EraseContents();}	///< Erase the content of current Excel cell. Set type to UNDEFINED.

	USHORT GetMergedRows() 				{return m_oExcelCell->GetMergedRows();}
	USHORT GetMergedColumns() 				{return m_oExcelCell->GetMergedColumns();}

	void SetMergedRows(USHORT mergedRows)		{m_oExcelCell->SetMergedRows(mergedRows);}
	void SetMergedColumns(USHORT mergedColumns)	{m_oExcelCell->SetMergedColumns(mergedColumns);}
...
public ref class ExcelWorksheet
	{
		class oExcelWorksheet *m_oExcelWorksheet;

	public:
		ExcelWorksheet() {} 

		ExcelWorksheet(ExcelWorksheet ^pExcelWorksheet) {}




		ExcelWorksheet(class oExcel *m_oExcel,int sheetIndex  ) {m_oExcelWorksheet = new oExcelWorksheet(m_oExcel,sheetIndex);}
		~ExcelWorksheet() {delete m_oExcelWorksheet;}

		void Attach(oExcelWorksheet * poExcelWorksheet)
		{
			m_oExcelWorksheet = poExcelWorksheet;
		}

		char* GetAnsiSheetName() 
			{return m_oExcelWorksheet->GetAnsiSheetName();}			///< Get the current worksheet name. Returns 0 if name is in Unicode format.
	
		wchar_t* GetUnicodeSheetName() {return m_oExcelWorksheet->GetUnicodeSheetName();}		///< Get the current worksheet name. Returns 0 if name is in Ansi format.
		bool GetSheetName(char* name) {return (m_oExcelWorksheet->GetUnicodeSheetName()!=NULL);}		///< Get the current worksheet name. Returns false if name is in Unicode format.
		bool GetSheetName(wchar_t* name) {return (m_oExcelWorksheet->GetSheetName(name)!=NULL);}	///< Get the current worksheet name. Returns false if name is in Ansi format.
		bool Rename(const char* to){return m_oExcelWorksheet->Rename(to);}		///< Rename current Excel worksheet to another ANSI name. Returns true if successful, false if otherwise.
		bool Rename(const wchar_t* to){return m_oExcelWorksheet->Rename(to);}		///< Rename current Excel worksheet to another Unicode name. Returns true if successful, false if otherwise.
		void Print(ostream& os) {m_oExcelWorksheet->Print(os);} ///< Print entire worksheet to an output stream, separating each column with the defined delimiter and enclosing text using the defined textQualifier. Leave out the textQualifier argument if do not wish to have any text qualifiers.
		void Print(ostream& os, char delimiter) {m_oExcelWorksheet->Print(os,delimiter);} ///< Print entire worksheet to an output stream, separating each column with the defined delimiter and enclosing text using the defined textQualifier. Leave out the textQualifier argument if do not wish to have any text qualifiers.
		void Print(ostream& os, char delimiter, char textQualifier) {m_oExcelWorksheet->Print(os,delimiter,textQualifier);} ///< Print entire worksheet to an output stream, separating each column with the defined delimiter and enclosing text using the defined textQualifier. Leave out the textQualifier argument if do not wish to have any text qualifiers.

public: // Cell functions
	    int GetTotalRows()  {return m_oExcelWorksheet->GetTotalRows();}		///< Total number of rows in current Excel worksheet.
		int GetTotalCols()  {return m_oExcelWorksheet->GetTotalCols();}		///< Total number of columns in current Excel worksheet.

		ExcelCell ^Cell(int row, int col){	ExcelCell ^myExcelCell = gcnew ExcelCell();
											oExcelCell  *poExcelCell;
											poExcelCell = m_oExcelWorksheet->Cell(row,col);
											myExcelCell->Attach(poExcelCell);
											return myExcelCell;
		                                  } ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.

		//oExcelCell* Cell(int row, int col) const; ///< Return a pointer to an Excel cell. row and col starts from 0. Returns 0 if row exceeds 65535 or col exceeds 255.
		bool EraseCell(int row, int col) {return m_oExcelWorksheet->EraseCell(row,col);}		///< Erase content of a cell. row and col starts from 0. Returns true if successful, false if row or col exceeds range.

		void SetColWidth(const int colindex , const USHORT colwidth){ m_oExcelWorksheet->SetColWidth(colindex,colwidth);}
		void SetRowHeight(const int rowindex, const USHORT rowheight){ m_oExcelWorksheet->SetRowHeight(rowindex,rowheight);}

		USHORT GetColWidth(const int colindex) {return m_oExcelWorksheet->GetColWidth(colindex);}

		void MergeCells(int row, int col, USHORT rowRange, USHORT colRange)  {return m_oExcelWorksheet->MergeCells(row,col,rowRange,colRange);}


	};

	public ref class Excel
	{
	public:

		class oExcel *m_oExcel;

		Excel() {m_oExcel = new oExcel();
				}
		~Excel() {delete m_oExcel;}

		bool NewSheet(int i);

		ExcelWorksheet  ^GetWorksheet(int i);

		bool SaveAs(System::String ^strFileName);

		bool ShellExecute_(System::String ^strFileName);
