...
Workbook::Font::Font() : Record(),
	height_(200), options_(EXCEL_FONT_OPTIONS::EXCEL_FONT_OPTIONS_NONE), colourIndex_(0x7FFF), weight_(400), escapementType_(EXCEL_ESCAPEMENT::EXCEL_ESCAPEMENT_NONE),
	underlineType_(EXCEL_UNDERLINE_FLAGS::EXCEL_UNDERLINE_NONE), family_(EXCEL_FONT_FAMILY::EXCEL_FONT_FAMILY_DONTCARE), characterSet_(0), unused_(0)
{
	code_ = CODE::FONT;
	dataSize_ = 14;
	recordSize_ = 18;
...
ULONG Workbook::Font::Read(const char* data)
{
	...
	LittleEndian::Read_EXCEL_FONT_OPTIONS(data_, options_, 2, 2);

	LittleEndian::Read(data_, colourIndex_, 4, 2);
	LittleEndian::Read(data_, weight_, 6, 2);

	LittleEndian::Read_EXCEL_ESCAPEMENT(data_, escapementType_, 8, 2);


	LittleEndian::Read_EXCEL_UNDERLINE_FLAGS(data_, underlineType_, 10, 1);

	LittleEndian::Read_EXCEL_FONT_FAMILY(data_, family_, 11, 1);
...
ULONG Workbook::Font::Write(char* data)
{
	...
	LittleEndian::Write_EXCEL_FONT_OPTIONS(data_, options_, 2, 2);
	LittleEndian::Write(data_, colourIndex_, 4, 2);
	LittleEndian::Write(data_, weight_, 6, 2);
	LittleEndian::Write_EXCEL_ESCAPEMENT(data_, escapementType_, 8, 2);
	LittleEndian::Write_EXCEL_UNDERLINE_FLAGS(data_, underlineType_, 10, 1);
	LittleEndian::Write_EXCEL_FONT_FAMILY(data_, family_, 11, 1);
...
Workbook::XF::XF() : Record(),
	...
	alignment_((EXCEL_ALIGNMENT)0x20), // EXCEL_HALIGN_GENERAL|EXCEL_VALIGN_BOTTOM
	rotation_(0x00),
...
ULONG Workbook::XF::Read(const char* data)
{
	...
	LittleEndian::Read_EXCEL_ALIGNMENT(data_, alignment_, 6, 1);
...
ULONG Workbook::XF::Write(char* data)
{
	...
	LittleEndian::Write_EXCEL_ALIGNMENT(data_, alignment_, 6, 1);
...
/************************************************************************************************************/
oExcel::oExcel() {}
oExcel::oExcel(const char* filename)
{
	Load(filename);
}
...
bool oExcel::Load(const char* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool oExcel::Load(const wchar_t* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool oExcel::SaveAs(const char* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
bool oExcel::SaveAs(const wchar_t* filename)
{
	if (file_.IsOpen())
		file_.Close();
...
oExcelWorksheet* oExcel::GetWorksheet(int sheetIndex)
{
...
oExcelWorksheet* oExcel::GetWorksheet(const char* name)
{
...
// Returns 0 if there is no Excel worksheet with the given name.
oExcelWorksheet* oExcel::GetWorksheet(const wchar_t* name)
{
	int maxWorksheets = (int) yesheets_.size();
...
oExcelWorksheet* oExcel::AddWorksheet(int sheetIndex)
{
	int sheetNo = (int)yesheets_.size() + 1;
	oExcelWorksheet* yesheet = 0;
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
oExcelWorksheet* oExcel::AddWorksheet(const char* name, int sheetIndex)
{
	int maxWorksheets = (int) yesheets_.size();
	...
	Worksheet* worksheet;
	oExcelWorksheet* yesheet;

	if (sheetIndex == -1) {
		workbook_.boundSheets_.push_back(Workbook::BoundSheet());
		worksheets_.push_back(Worksheet());
		yesheets_.push_back(new oExcelWorksheet(this, (int)worksheets_.size()-1));
		...
		yesheet = &**(yesheets_.insert(yesheets_.begin()+sheetIndex, new oExcelWorksheet(this, sheetIndex)));
...
// Worksheet is added to the last position if sheetIndex == -1.
// Returns a pointer to the worksheet if successful, 0 if otherwise.
oExcelWorksheet* oExcel::AddWorksheet(const wchar_t* name, int sheetIndex)
{
	int maxWorksheets = (int)yesheets_.size();
...
bool oExcel::DeleteWorksheet(int sheetIndex)
{
...
// Returns true if successful, false if otherwise.
bool oExcel::DeleteWorksheet(const char* name)
{
	int maxWorksheets = (int)yesheets_.size();
...
// Returns true if successful, false if otherwise.
bool oExcel::DeleteWorksheet(const wchar_t* name)
{
...
char* oExcel::GetAnsiSheetName(int sheetIndex)
{
...
wchar_t* oExcel::GetUnicodeSheetName(int sheetIndex)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
...
bool oExcel::GetSheetName(int sheetIndex, char* name)
{
	if (!(workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1))
	{
...
bool oExcel::GetSheetName(int sheetIndex, wchar_t* name)
{
	if (workbook_.boundSheets_[sheetIndex].name_.unicode_ & 1)
	{
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool oExcel::RenameWorksheet(int sheetIndex, const char* to)
{
	int maxWorksheets = (int)yesheets_.size();
...
// Index starts from 0.
// Returns true if successful, false if otherwise.
bool oExcel::RenameWorksheet(int sheetIndex, const wchar_t* to)
{
	int maxWorksheets = (int)yesheets_.size();
	if (sheetIndex < maxWorksheets) {
		for(int i=0; i<maxWorksheets; ++i) {
...
// Returns true if successful, false if otherwise.
bool oExcel::RenameWorksheet(const char* from, const char* to)
{
	int maxWorksheets = (int)yesheets_.size();
	for(int i=0; i<maxWorksheets; ++i) {
		if (workbook_.boundSheets_[i].name_.unicode_ & 1)
...
// Returns true if successful, false if otherwise.
bool oExcel::RenameWorksheet(const wchar_t* from, const wchar_t* to)
{
	int maxWorksheets = (int) worksheets_.size();
...
size_t oExcel::Read(const char* data, size_t dataSize)
{
	size_t bytesRead = 0;
...
void oExcel::AdjustStreamPositions()
{
...
void oExcel::AdjustBoundSheetBOFPositions()
{
...
void oExcel::AdjustDBCellPositions()
{
...
void oExcel::AdjustExtSSTPositions()
{
...
void oExcel::UpdateYExcelWorksheet()
{
	int maxWorksheets = (int) worksheets_.size();
	yesheets_.clear();
	...
	for(int i=0; i<maxWorksheets; ++i) {
		yesheets_.push_back(new oExcelWorksheet(this, i));
...
void oExcel::UpdateWorksheets()
{
	...
	for(int s=0; s<maxWorksheets; ++s) {
		const oExcelWorksheet& sheet = *yesheets_[s];
		...
		for(int r=0, curRowBlock=0; r<maxRows; ++r) {
			...
			for(int c=0; c<maxCols; ++c) {
				const oExcelCell* cell = sheet.Cell(r, c);

				int cellType = cell->Type();
//				if (cellType != oExcelCell::UNDEFINED)	// Current cell contains some data
				// Write cell content, even if blank in order to keep format
				{
					...
					switch(cellType) {
						case oExcelCell::INT:
						{
							...
							int cl = c + 1;
							for(; cl<maxCols; ++cl) {
								const oExcelCell* cellNext = sheet.Cell(r, cl);
								if (cellNext->Type()==oExcelCell::UNDEFINED ||
		...
		for(int mr=0; mr<maxRows; ++mr) {
			for(int c=0; c<maxCols; ++c) {
				const oExcelCell* cell = sheet.Cell(mr,c);
...
/************************************************************************************************************/
oExcelWorksheet::oExcelWorksheet(oExcel* excel, int sheetIndex) :
...
// Returns 0 if name is in Unicode format.
char* oExcelWorksheet::GetAnsiSheetName()
{
...
// Get the current worksheet name.
// Returns 0 if name is in Ansi format.
wchar_t* oExcelWorksheet::GetUnicodeSheetName()
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
...
// Get the current worksheet name.
// Returns false if name is in Unicode format.
bool oExcelWorksheet::GetSheetName(char* name)
{
	if (!(excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1))
	{
...
// Get the current worksheet name.
// Returns false if name is in Ansi format.
bool oExcelWorksheet::GetSheetName(wchar_t* name)
{
	if (excel_->workbook_.boundSheets_[sheetIndex_].name_.unicode_ & 1)
	{
...
// Returns true if successful, false if otherwise.
bool oExcelWorksheet::Rename(const char* to)
{
...
// Returns true if successful, false if otherwise.
bool oExcelWorksheet::Rename(const wchar_t* to)
{
	int maxWorksheets = (int) excel_->workbook_.boundSheets_.size();
	for(int i=0; i<maxWorksheets; ++i) {
		if (!(excel_->workbook_.boundSheets_[i].name_.unicode_ & 1)) continue;
...
void oExcelWorksheet::Print(ostream& os, char delimiter, char textQualifier) const
{
	for(int r=0; r<maxRows_; ++r)
	{
		for(int c=0; c<maxCols_; ++c)
		{
			const oExcelCell* cell = Cell(r, c);
			switch(cell->Type())
			{
				case oExcelCell::UNDEFINED:
					break;
...
int oExcelWorksheet::GetTotalRows() const
{
	return maxRows_;
}

// Total number of columns in current Excel worksheet.
int oExcelWorksheet::GetTotalCols() const
{
	return maxCols_;
}
...
oExcelCell* oExcelWorksheet::Cell(int row, int col)
{
	...
	if (row >= maxRows_) {
		...
		maxRows_ = row + 1;
		cells_.resize(maxRows_, vector<oExcelCell>(maxCols_));
	}
...
// row and col starts from 0.
// Returns NULL if row exceeds 65535 or col exceeds 255.
const oExcelCell* oExcelWorksheet::Cell(int row, int col) const
{
...
bool oExcelWorksheet::EraseCell(int row, int col)
{
...
void oExcelWorksheet::SetColWidth(const int colindex, const USHORT colwidth)
{
	//MessageBox(NULL,L"In SetColWidth()",L"ERROR",MB_OK);
//	System::Windows::Forms::MessageBox::Show("In SetColWidth()");
...
void oExcelWorksheet::SetRowHeight(const int rowindex, const USHORT rowheight)
{
	Worksheet::CellTable *pcellTable;

	pcellTable = &excel_->worksheets_[sheetIndex_].cellTable_;
	size_t maxRowBlocks_ = pcellTable->rowBlocks_.size();

	for(size_t j=0; j<maxRowBlocks_; ++j) 
	{
		size_t maxRows = pcellTable->rowBlocks_[j].rows_.size();
		for(size_t k=0; k<maxRows; ++k)
		{
			if (pcellTable->rowBlocks_[j].rows_[k].rowIndex_ == rowindex)
			{
				pcellTable->rowBlocks_[j].rows_[k].height_ = rowheight;
			}
		}
	}
...
USHORT oExcelWorksheet::GetColWidth(const int colindex)
{
...
// Update cells using information from oExcel.worksheets_
void oExcelWorksheet::UpdateCells()
{
...
//	maxCols_ = dimension.lastUsedColIndexPlusOne_;
	...
	vector<oExcelCell> *pcellCol = new vector<oExcelCell>(maxCols_);

	cells_.resize(maxRows_,  *pcellCol);
...
void oExcelWorksheet::MergeCells(int row, int col, USHORT rowRange, USHORT colRange)
{
	oExcelCell* cell = Cell(row, col);

	cell->SetMergedRows(rowRange);
	cell->SetMergedColumns(colRange);

/* Assembling MERGECELL records is accomplished in oExcel::UpdateWorksheets()
	Worksheet::MergedCells::MergedCell mergedCell;
...
int oExcelCell::Type() const {return type_;}

// Get an integer value.
// Returns false if cell does not contain an integer or a double.
bool oExcelCell::Get(int& val) const
{
	if (type_ == INT) {
		val = ival_;
		return true;
...
bool oExcelCell::Get(double& val) const
{
	if (type_ == DOUBLE) {
		val = dval_;
		return true;
...
bool oExcelCell::Get(char* str) const
{
	if (type_ == STRING) {
		if (str_.empty()) *str = '\0';
...
bool oExcelCell::Get(wchar_t* str) const
{
	if (type_ == WSTRING) {
		if (wstr_.empty())
			*str = L'\0';
...
size_t oExcelCell::GetStringLength() const
{
	if (type_ == STRING)
		return str_.size() - 1;
	else {
...
int oExcelCell::GetInteger() const
{
	int val;
...
double oExcelCell::GetDouble() const
{
	double val;
...
const char* oExcelCell::GetString() const
{
...
const wchar_t* oExcelCell::GetWString() const
{
...
void oExcelCell::Set(int val)
{
	SetInteger(val);
}

// Set content of current Excel cell to a double.
void oExcelCell::Set(double val)
{
	SetDouble(val);
}

// Set content of current Excel cell to an ANSI string.
void oExcelCell::Set(const char* str)
{
	SetString(str);
}

// Set content of current Excel cell to an Unicode string.
void oExcelCell::Set(const wchar_t* str)
{
	SetWString(str);
}

// Set content of current Excel cell to an integer.
void oExcelCell::SetInteger(int val)
{
	type_ = INT;
	ival_ = val;
}
...
// Set content of current Excel cell to a double.
void oExcelCell::SetDouble(double val)
{
	type_ = DOUBLE;
	dval_ = val;
}
...
void oExcelCell::SetRKValue(int rkValue)
{
...
// Set content of current Excel cell to an ANSI string.
void oExcelCell::SetString(const char* str)
{
...
// Set content of current Excel cell to an Unicode string.
void oExcelCell::SetWString(const wchar_t* str)
{
...
//MF
void oExcelCell::SetFormula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f)
{
	type_ = oExcelCell::FORMULA;
	_pFormula = new Formula(f);
}

oExcelCell::Formula::Formula(const Worksheet::CellTable::RowBlock::CellBlock::Formula& f)
 :	  _formula_type(0)
{
...
bool oExcelCell::get_formula(Worksheet::CellTable::RowBlock::CellBlock* pCell) const
{
...
ostream& operator<<(ostream& os, const oExcelCell& cell)
{
	switch(cell.Type())
	{
		case oExcelCell::UNDEFINED:
	...
	ExcelWorksheet  ^Excel::GetWorksheet(int i)
	{
		ExcelWorksheet ^myExcelWorksheet = gcnew ExcelWorksheet();

		oExcelWorksheet  *poExcelWorksheet;
		poExcelWorksheet = m_oExcel->GetWorksheet(i);
		myExcelWorksheet->Attach(poExcelWorksheet);
		return myExcelWorksheet;
	}
	
	bool Excel::SaveAs(System::String ^strFileName)
	{
		pin_ptr<const wchar_t> wch = PtrToStringChars(strFileName);
		wchar_t wcstring[MAX_PATH];
		wcscpy_s(wcstring, wch);
		//wcscat_s(wcstring, L" (wchar_t *)");
		return m_oExcel->SaveAs(wcstring);
	}
	
	bool Excel::ShellExecute_(System::String ^strFileName)
	{
    ...
    // string, including space for a terminating null.
		...
		HINSTANCE h;
		h = ShellExecute(0, NULL, wcstring, NULL, NULL, SW_NORMAL);
		if (h==0)
		{
			return true;
		}
	...
	void  ExcelCell::SetFormat(CellFormat ^fmt)
	{
		m_oExcelCell->SetFormat(fmt->m_poCellFormat);
	}
