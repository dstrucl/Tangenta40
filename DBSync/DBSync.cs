...
namespace DBSync
{
    public static class DBSync
    {
        ...
        public static MyDataBase_Blagajna DB_for_Blagajna = null;
        ...
        {
            return DB_for_Blagajna.DropViews();
        }

        public static bool Create_VIEWs()
        {
            return DB_for_Blagajna.Create_VIEWs();
        }
        ...
        public static bool Init(Form parentform, string m_XmlFileName, string IniFileFolder, ref string DataBaseType,bool bShowDialog)
        {
            ...
            {
                DB_for_Blagajna = new BlagajnaDataBaseDef.MyDataBase_Blagajna(parentform, m_XmlFileName, IniFileFolder);
            }
            DBConnection.eDBType org_m_DBType = m_DBType; 
            m_DBType = Get_DBType(ref DataBaseType, bShowDialog);
            DB_for_Blagajna.Init(m_DBType);
            ...
            if (DBSync.m_DBType == DBConnection.eDBType.SQLITE)
            {
                ...
                {
                    LogFile.Error.Show("Napaka:Ne morem naložiti podatkovne baze za Blagajno: Err=" + Err);
                    return false;
                }
            }
            else
        ...
        {
            get { return DB_for_Blagajna.m_DBTables.m_con.DataBase; }
        }

        public static string ServerType
        {
            get { return DB_for_Blagajna.m_DBTables.m_con.ServerType; }
        }
        ...
        public static string DataSource
        {
            get
            {
                if (DB_for_Blagajna.m_DBTables.m_con.DBType == DBConnection.eDBType.SQLITE)
                {
                    return DB_for_Blagajna.m_DBTables.m_con.Settings_LocalDB.LocalDB_DataBaseFilePath() + "\\" + DB_for_Blagajna.m_DBTables.m_con.Settings_LocalDB.LocalDB_DataBaseFileName();
                }
                else
                {
                    return DB_for_Blagajna.m_DBTables.m_con.Settings_RemoteDB.DataSource();
                }
            }
        }
        ...
        {
            get { return DB_for_Blagajna.m_DBTables.m_con.DataBase_BackupTemp; }
        }

        public static bool ReadDataTable(ref DataTable dt,string sql,ref string Err)
        {
            return DB_for_Blagajna.m_DBTables.m_con.ReadDataTable(ref dt, sql, ref Err);
        }

        public static bool ReadDataTable(ref DataTable dt, string sql,List<SQL_Parameter> lpar, ref string Err)
        {
            return DB_for_Blagajna.m_DBTables.m_con.ReadDataTable(ref dt, sql,lpar, ref Err);
        }

        public static bool ExecuteNonQuerySQL(string sql, List<SQL_Parameter> lpar,ref object ores,ref string Err)
        {
            return DB_for_Blagajna.m_DBTables.m_con.ExecuteNonQuerySQL(sql, lpar, ref ores, ref Err);
        }

        public static bool ExecuteNonQuerySQL_NoMultiTrans(string sql, List<SQL_Parameter> lpar,ref string Err)
        {
            return DB_for_Blagajna.m_DBTables.m_con.ExecuteNonQuerySQL_NoMultiTrans(sql, lpar, ref Err);
        }
        ...
        {
            return DB_for_Blagajna.m_DBTables.m_con.ExecuteNonQuerySQLReturnID(sql, lpar, ref id, ref oret, ref Err, sTableName);
        }
        ...
        public static bool CreateTables(string[] new_tables)
        {
            ...
            {
                SQLTable tbl = DB_for_Blagajna.m_DBTables.GetTable(stbl);
                if (tbl != null)
                {
                    if (DB_for_Blagajna.m_DBTables.m_con.ExecuteNonQuerySQL_NoMultiTrans(tbl.sql_CreateTable,null, ref Err))
                    {
                        continue;
                    }
                    else
        ...
        public static bool Get(Form parent, ref string Err, ref string inifile_prefix, string default_DataBase_name)
        {
            ...
            if (DBSync.DB_for_Blagajna.m_DBTables.MakeDataBaseConnection(parent, DBSync.RemoteDB_data))
            {
